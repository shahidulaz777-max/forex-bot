...
user_lockout = {}
active_trades = {}
monitoring_pairs = {}  # অটো-স্ক্যানিং ও ব্যাকগ্রাউন্ড মনিটরিংয়ের জন্য
...
# ==================== WORLD-CLASS AI MULTI-INDICATOR & NEWS ENGINE ====================
...
def analyze_forex_market(pair_name):
    ...
    if df_5m.empty or df_15m.empty:
        return "INVALID_PAIR", f"❌ **'{pair_name.upper()}'** ফরেক্স পেয়ারটির রিয়েল মার্কেট লাইভ ডাটা এই মুহূর্তে পাওয়া যায়নি। সঠিক ফরেক্স পেয়ারের নাম লিখুন (যেমন: `EURUSD`, `XAUUSD`, `GBPJPY` ইত্যাদি)।"
    ...
    is_news_spike = current_atr > (avg_atr * 2.2) if not np.isnan(avg_atr) else False

    if is_news_spike:
        return "NO_TRADE", (
            f"🚨 **HIGH-IMPACT NEWS / VOLATILITY ALERT** 🚨\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📍 **Pair:** `{pair_name.upper()}`\n"
            f"💵 **Price:** `{curr_price:.5f}`\n\n"
            f"⚠️ **সতর্কতা:** এই মুহূর্তে মার্কেটে বড় ধরনের নিউজ বা অতিরিক্ত ভোলাটিলিটি চলছে। তাই ট্রেড করা সম্ভব নয়। **বট ব্যাকগ্রাউন্ডে মার্কেট মনিটর করছে, অনুকূল জোন আসা মাত্র অটো সিগন্যাল পাঠানো হবে।**"
        )
    ...
    is_bullish_momentum = (rsi_5m > 55) and (rsi_15m > 50) and (macd_5m > sig_5m)

    is_bearish_trend = (curr_price < ema20_5m < ema50_5m) and (ema20_15m < ema50_15m)
    is_bearish_momentum = (rsi_5m < 45) and (rsi_15m < 50) and (macd_5m < sig_5m)
    ...
    if is_bullish_trend and is_bullish_momentum:
        ...
        trend_status = "Uptrend (আপট্রেন্ড)" if curr_price > ema20_5m else "Downtrend (ডাউনট্রেন্ড)"
        return "NO_TRADE", (
            f"🛡️ **HIGH-ACCURACY FILTER: NO TRADE**\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📍 **Pair:** `{pair_name.upper()}`\n"
            f"💵 **Price:** `{curr_price:.5f}`\n"
            f"📈 **Current Market Trend:** `{trend_status}`\n\n"
            f"💡 **কারণ:** মার্কেট বর্তমানে `{trend_status}` এ রয়েছে, কিন্তু সঠিক কনফার্মেশন জোন ম্যাচ করছে না। **বট ব্যাকগ্রাউন্ডে এনালাইজ চালিয়ে যাচ্ছে, ভালো জোন পেলেই অটো সিগন্যাল চলে আসবে।**"
        )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "আপনার মোট **ক্যাপিটাল / ব্যালেন্স** কত? (আপনার ইচ্ছামতো যেকোনো সংখ্যা লিখতে পারেন যেমন: `100`, `1000`, `150000`):",
        parse_mode="Markdown"
    )
    return ASK_CAPITAL
...
async def set_capital(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(
            ...
            f"প্রতিটি ট্রেডে আপনি কত পার্সেন্ট **রিস্ক (Risk %)** নিতে চান? (যেমন: `1`, `2`, `5`):",
            parse_mode="Markdown"
        )
        return ASK_RISK
    except ValueError:
...
async def process_pair(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pair_name = update.message.text.strip().upper()
    capital = user_capital.get(user_id, 1000.0)
    risk_pct = user_risk.get(user_id, 1.0)

    wait_msg = await update.message.reply_text(f"🔍 **{pair_name}** মার্কেট স্ক্যান করা হচ্ছে এবং ব্যাকগ্রাউন্ড অটো-মনিটরিং চালু করা হলো...", parse_mode="Markdown")

    res = analyze_forex_market(pair_name)

    # ইউজারের পেয়ারটি ব্যাকগ্রাউন্ড মনিটরিং লিস্টে সেভ করে রাখা হলো যাতে অটো সিগন্যাল দিতে পারে
    ...
    await update.message.reply_text("👉 পটভূমিতে মার্কেট স্ক্যান চলছে। অন্য পেয়ার দেখতে চাইলে নতুন নাম লিখে পাঠান:")
    return ASK_PAIR

# ==================== BACKGROUND CONTINUOUS MARKET MONITOR & AUTO-SIGNAL ====================

async def background_market_scanner(app):
    while True:
        try:
            await asyncio.sleep(30) # প্রতি ৩০ সেকেন্ড পর পর ব্যাকগ্রাউন্ডে চেক করবে
            for uid, info in list(monitoring_pairs.items()):
                # যদি ইউজারের অলরেডি কোনো লাইভ ট্রেড চলতে থাকে, তবে স্কিপ করবে
                if uid in active_trades:
                    continue
                
                # যদি ইউজার লকআউটে থাকে, স্কিপ করবে
                ...
                if res[0] in ["BUY", "SELL"]:
                    ...
                    msg = (
                        f"⚡ **AUTO-TRIGGERED HIGH-ACCURACY {sig_type} SIGNAL** ⚡\n"
                        f"━━━━━━━━━━━━━━━━━━━━━━\n"
                        f"📍 **Pair:** `{pair_name}`\n"
                        f"⏱️ **Trade Expiry:** `5 Minutes`\n\n"
                        f"📌 **Copy & Paste Trading Entries:**\n"
                    ...
                    try:
                        ...
                        ax.set_title(f"{pair_name} - Auto 5M Entry Signal Analysis", color='white', fontsize=10, fontweight='bold')
                        ax.legend(loc='upper left', facecolor='#1e1e1e', edgecolor='none', labelcolor='white', fontsize=7)
                        ax.grid(True, color='#333333', linestyle=':', alpha=0.6)
                    ...
                    # সিগন্যাল পেয়ে গেলে আপাতত মনিটরিং থেকে রিমুভ হবে (ট্রেড শেষ হলে আবার মনিটরিং এ আসবে)
            ...
            for uid, trade in list(active_trades.items()):
                if now >= trade["expiry_time"]:
                    ...
                    if not df.empty:
                        ...
                        await app.bot.send_message(chat_id=chat_id, text="👉 নতুন পেয়ারের নাম লিখে পাঠান অথবা আগের পেয়ার আবার মনিটরিংয়ে রাখতে নতুন করে টাইপ করুন:", parse_mode="Markdown")
...
async def main():
    ...
    while True:
        ...
        try:
            ...
            asyncio.create_task(background_market_scanner(app)) # ব্যাকগ্রাউন্ড স্ক্যানার চালু করা হলো
            print("🚀 Institutional High-Accuracy AI Cloud Bot & Continuous Scanner is running 24/7...")
            
            while True:
                await asyncio.sleep(1)

        except Exception as e:
            print(f"Reconnecting due to network interruption: {e}")
            if app:
                try:
